...
#include "protocol/deps/base/TypeTraits.h"
...
constexpr bool tuple_like_impl =
    traits::is_specialization_of_v<T, ::std::tuple> ||
    traits::is_specialization_of_v<T, ::std::pair> ||
    traits::is_std_array_v<T> || traits::is_subrange_v<T>;

template <class T>
concept TupleLike =
    tuple_like_impl<std::remove_cvref_t<T>> || (!Rangeable<T> && requires(T t) {
...
template <class T>
concept ArrayLike =
    Rangeable<T> && !requires { typename std::remove_cvref_t<T>::mapped_type; };

template <class T, template <class...> class Z>
concept DerivedFromSpecializes =
    traits::is_derived_from_specialization_of_v<T, Z>;
