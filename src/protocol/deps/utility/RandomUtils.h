...
template <std::integral T> inline T rand(T upBound = 0) {
...
template <std::floating_point T> inline T rand() {
  union {
    uintmax_t u;
    T f;
  } x;
...
template <std::floating_point T> inline T rand(T max) {
  return rand<T>() * max;
}
template <class T> inline T rand(T min, T max) {
  return min + rand<T>(max - min);
}
template <class T> inline T openIntervalRand(T min = 0, T max = 1) {
  return rand<T>(std::nexttoward(min, std::numeric_limits<long double>::max()),
                 max);
}
template <class T> inline T closeIntervalRand(T min = 0, T max = 1) {
  return rand<T>(min,
                 std::nexttoward(max, std::numeric_limits<long double>::max()));
}
template <class T> inline T rightCloseIntervalRand(T min = 0, T max = 1) {
  return rand<T>(std::nexttoward(min, std::numeric_limits<long double>::max()),
                 std::nexttoward(max, std::numeric_limits<long double>::max()));
}
template <class T> inline T leftCloseIntervalRand(T min = 0, T max = 1) {
