...
public:
  ...
  [[nodiscard]] constexpr HashedString(std::nullptr_t = nullptr) noexcept
      : hash(0), lastMatch(nullptr) {} // NOLINT

  [[nodiscard]] constexpr HashedString(uint64 h, char const *str) noexcept
      : hash(h), str(str), lastMatch(nullptr) {}

  [[nodiscard]] constexpr HashedString(std::string_view str) noexcept
      : hash(computeHash(str)), str(str), lastMatch(nullptr) {} // NOLINT

  [[nodiscard]] constexpr HashedString(std::string &&s) noexcept // NOLINT
      : hash(computeHash(s)), str(std::move(s)), lastMatch(nullptr) {}
  [[nodiscard]] constexpr HashedString(char const *str) noexcept
      : HashedString(std::string{str}) {} // NOLINT
  ...
  [[nodiscard]] constexpr HashedString(HashedString &&other) noexcept
      : hash(other.hash), str(std::move(other.str)),
        lastMatch(other.lastMatch) {
    other.hash = 0;
    other.lastMatch = nullptr;
  }

  [[nodiscard]] constexpr HashedString &
  operator=(HashedString const &other) noexcept {
    if (this == &other) {
      return *this;
    }
    hash = other.hash;
  ...
  [[nodiscard]] constexpr HashedString &
  operator=(HashedString &&other) noexcept {
    if (this == &other) {
      return *this;
    }
    hash = other.hash;
  ...
  operator==(StringType const &rhs) const noexcept {
    return str == rhs;
  }
...
template <> struct hash<protocol::HashedString> {
  size_t operator()(protocol::HashedString const &str) const noexcept {
    return str.getHash();
  }
};
